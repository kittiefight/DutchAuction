...
contract('KittiefightToken', function(accounts) {
  ...
  describe('isTransferWhitelistOnly()', function(){
    ...
    before(async function() {
      const toMint = Math.pow(10, 24);
      await kittiefightToken.mint(tokenOwner, toMint);
      const accountBalance = await kittiefightToken.balanceOf.call(tokenOwner);

      assert.equal(toMint, accountBalance.toNumber(), `Failed to mint correct number of tokens to ${tokenOwner}`)
    })
    ...
    describe('transfer()', function() {
      ...
      it('should fail to transfer from non-Whitelisted', async function() {
        ...
        try {
          await kittiefightToken.transfer(accounts[3], Math.pow(10,20), {
            from: tokenOwner
          });
          assert.isNotOk(true, 'Expected function to fail');
        } catch(e) {
      ...
      it('should transfer from whiteListed', async function() {
        ...
        try {
          await kittiefightToken.transfer(receiver, Math.pow(10,20), {
            from: tokenOwner
          });

          const newBalance = await kittiefightToken.balanceOf.call(receiver);
          assert.deepEqual(balance.plus(Math.pow(10,20)), newBalance, 'Wrong amount of tokens transferred to receiver');
        } catch(e) {
          assert.isNotOk(true, `Expected function to complete: ${e.message || e}`);
        };
      })
    ...
    describe('transferFrom()', function() {
      ...
      before(async function () {
        ...
        assert.equal(toSend, allowance.toNumber(), 'Wrong amount of tokens approved to spender');
      })
      ...
      it('should transferFrom whiteListed', async function() {
        ...
        try {
          ...
          const newBalance = await kittiefightToken.balanceOf.call(receiver);
          assert.deepEqual(balance.plus(toSend), newBalance, 'Wrong amount of tokens transferred to receiver');
        } catch(e) {
          assert.isNotOk(true, `Expected function to complete: ${e.message || e}`);
        };
      })
